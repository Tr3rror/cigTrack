      ...
      <View style={styles.calendarGrid}>
        {calendarDays.map((d) => {
          ...
          return (
            <TouchableOpacity
              key={d}
              style={[styles.dayCell, { backgroundColor: colors.card, borderColor: value ? colors.primary : '#3331', borderWidth: value ? 2 : 1 }]}
              onPress={() => onDayPress(d)}
            >
              <Text style={{ color: colors.text, fontSize: 10 }}>{d.split('-')[2]}</Text>
              <Text style={{ fontSize: 16, fontWeight: '900', color: value ? colors.text : colors.accent, opacity: value ? 1 : 0.2 }}>
                {value || '-'}
              </Text>
            </TouchableOpacity>
          );
...
const styles = StyleSheet.create({
  ...
  dayCell: { width: '17.5%', aspectRatio: 1, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
});